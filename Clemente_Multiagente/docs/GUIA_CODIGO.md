...
| RAG en Azure | `app/agentes/rag/indice.py` | Cada chunk se identifica por archivo, seccion y numero dentro de la seccion (no por su posicion global); reindexar actualiza en su lugar y borra lo que ya no esta en el catalogo, salvo con un catalogo vacio o una carga incompleta. El indice de Azure debe existir con los campos `id`, `document`, `section`, `content` y `content_vector` de 1536 dimensiones: el codigo no lo crea. |
| Limites por conversacion | `app/orquestador/limites.py`, `app/orquestador/grafo.py:responder` | Rechaza mensajes de mas de 2000 caracteres y rafagas de mas de 12 por minuto sin llamar al modelo, y serializa los turnos de una misma conversacion. Viven en memoria del proceso: valen por replica. |
| Errores y trazas sin detalle | `app/seguridad/errores.py`, `app/seguridad/trazado.py` | Las trazas y las tools guardan solo el tipo de la excepcion (el texto queda en el log del servidor); LangSmith recibe inputs y outputs redactados y el thread_id del checkpoint es un hash, no el telefono. |
